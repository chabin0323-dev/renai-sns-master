...
function convertEnglishToKatakana(text) {
  if (!text) return text;
  let result = text;
  for (const item of ENGLISH_TO_KATAKANA_MAP) {
    result = result.replace(item.pattern, item.katakana);
  }
  return result;
}
 ...
 * 全項目を空のまま返す。以前はコードフェンス（3連バッククォートで囲まれたブロック）を
 * 探すフォールバック処理があったが、その正規表現がファイル貼り付け時の破損の
 * 原因になりやすかったため削除した。
 */
